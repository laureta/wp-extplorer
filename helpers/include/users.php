...
/**
 * @version $Id: users.php 107 2008-07-22 17:27:12Z soeren $
 * @package eXtplorer
...
function &find_user($user,$pass) {
	...
	for($i=0;$i<$cnt;++$i) {
		if($user==$GLOBALS["users"][$i][0]) {
			//KJ. Commented the following
			/*if($pass==NULL || ($pass==$GLOBALS["users"][$i][1] &&
				$GLOBALS["users"][$i][7]))
			{
				return $GLOBALS["users"][$i];
			}*/
			//KJ. Added the following
			$ph = new PasswordHash(8,TRUE);
			$passed = $ph->CheckPassword($pass,$GLOBALS["users"][$i][1]);
			if($passed){
				$GLOBALS["users"][$i][1] = $pass;
				return $GLOBALS["users"][$i];
			}
		}
	}
